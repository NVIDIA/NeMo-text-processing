...
class TimeFst(GraphFst):
    ...
    def __init__(self):
        ...
        midnight_to_hour_graph = pynini.string_file(
            get_abs_path("data/time/midnight_to_hour.tsv")
        )
        to_hour_graph = pynini.string_file(get_abs_path("data/time/to_hour.tsv"))

        minute_verbose_graph = pynini.string_file(
            get_abs_path("data/time/minute_verbose.tsv")
        )
        minute_to_graph = pynini.string_file(get_abs_path("data/time/minute_to.tsv"))
        minute_to_verbose_graph = pynini.string_file(
            get_abs_path("data/time/minute_to_verbose.tsv")
        )
        ...
        time_word = "דקות"
        optional_delete_time = pynini.closure(
            delete_space + pynutil.delete(time_word), 0, 1
        )
        graph_h_and_m = (
            final_graph_hour
            + delete_space
