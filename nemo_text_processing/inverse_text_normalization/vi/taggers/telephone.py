...
class TelephoneFst(GraphFst):
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        vietnamese_mobile = pynini.compose(
            ...
            pynini.accep("0") + NEMO_DIGIT**2 + pynutil.insert("-") + NEMO_DIGIT**3 + pynutil.insert("-") + NEMO_DIGIT**4
        )
        ...
        country_code_digits = pynini.union(
            pynutil.add_weight(cardinal_two_digit, -0.001),
            digit + delete_space + digit,
            digit
        )
        ...
        ip_octet = pynini.union(
            pynini.closure(digit + delete_space, 0, 2) + digit,
            cardinal_two_digit
        )
        
        ip_graph = (
            ip_octet
            + (delete_space + pynini.cross("chấm", ".") + delete_space + ip_octet) ** 3
        )
        ...
        card_16 = pynini.compose(
            sixteen_digits,
            NEMO_DIGIT**4 + pynutil.insert(" ") + NEMO_DIGIT**4 + pynutil.insert(" ") + NEMO_DIGIT**4 + pynutil.insert(" ") + NEMO_DIGIT**4
        )
        
        fifteen_digits = pynini.closure(digit + delete_space, 14) + digit
        card_15 = pynini.compose(
            fifteen_digits,
            NEMO_DIGIT**4 + pynutil.insert(" ") + NEMO_DIGIT**6 + pynutil.insert(" ") + NEMO_DIGIT**5
        )
