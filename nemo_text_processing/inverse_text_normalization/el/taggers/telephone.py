...
class TelephoneFst(GraphFst):
    def __init__(self, cardinal: GraphFst, input_case: str = INPUT_LOWER_CASED):
        ...
        digit_to_str = pynini.invert(pynini.string_file(get_abs_path("data/numbers/digit.tsv")).optimize())
        ...
        number_part = pynini.compose(
            str_to_digit + pynini.closure(delete_space + str_to_digit), NEMO_DIGIT**10 + pynini.closure(NEMO_DIGIT)
        )
