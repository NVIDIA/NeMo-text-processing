    ...
    def __init__(
        ...
        if cache_dir is not None and cache_dir != "None":
            ...
            far_file = os.path.join(
                cache_dir, f"_{input_case}_sv_tn_{deterministic}_deterministic_{whitelist_file}_lm.far"
            )
        if not overwrite_cache and far_file and os.path.exists(far_file):
            ...
            fraction = FractionFst(deterministic=True, ordinal=ordinal, cardinal=cardinal)
            fraction_graph = fraction.fst
            measure = MeasureFst(cardinal=cardinal, decimal=decimal, fraction=fraction, deterministic=True)
            measure_graph = measure.fst
            date_graph = DateFst(cardinal=cardinal, ordinal=ordinal, deterministic=True).fst
            ...
            v_telephone_graph = vTelephoneFst(deterministic=True).fst
            v_electronic_graph = vElectronicFst(deterministic=True).fst
