...
class MoneyFst(GraphFst):
    """Verbalizes Polish currency tokens."""

    def __init__(self, deterministic: bool = True):
        super().__init__(name="money", kind="verbalize", deterministic=deterministic)
        value = pynini.closure(NEMO_NOT_QUOTE, 1)
        integer = pynutil.delete('integer: "') + value + pynutil.delete('"')
        currency = pynutil.delete('currency: "') + value + pynutil.delete('"')
        ...
        self.fst = self.delete_tokens(
            (integer | decimal | fraction) + delete_space + pynutil.insert(" ") + currency
        ).optimize()
