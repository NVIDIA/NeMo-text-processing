...
def get_quantity(decimal_graph: "pynini.FstLike", cardinal_graph: "pynini.FstLike") -> "pynini.FstLike":
    ...
    e.g. २ लाख -> integer_part: "दो" quantity: "लाख"
    e.g. २.४ लाख -> integer_part: "दो" fractional_part: "चार" quantity: "लाख"
    e.g. २.४॰॰ लाख -> integer_part: "दो हज़ार चार सौ" fractional_part: "चार" quantity: "लाख"
       ...
       # )
        ...
        if not deterministic:
            no_oh_zero = pynini.difference(
                NEMO_SIGMA,
                (NEMO_SIGMA + "oh" + NEMO_SIGMA + "शून्य" + NEMO_SIGMA)
                | (NEMO_SIGMA + "शून्य" + NEMO_SIGMA + "oh" + NEMO_SIGMA),
            ).optimize()
            no_zero_oh = pynini.difference(
                NEMO_SIGMA, NEMO_SIGMA + pynini.accep("शून्य") + NEMO_SIGMA + pynini.accep("oh") + NEMO_SIGMA
            ).optimize()

            self.final_graph_wo_negative |= pynini.compose(
                self.final_graph_wo_negative,
                pynini.cdrewrite(
                    pynini.cross("integer_part: \"शून्य \"", "integer_part: \"oh\""), NEMO_SIGMA, NEMO_SIGMA, NEMO_SIGMA
                ),
            )
