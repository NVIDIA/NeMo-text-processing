...
class MoneyFst(GraphFst):
    ...
    def __init__(self, decimal: GraphFst, deterministic: bool = True):
        ...
        mandarin_currency_component = (
            pynutil.delete("mandarin_currency: \"") + pynini.closure(currencies) + pynutil.delete("\"")
        )
        decimal_component = decimal.decimal_component
        unit_only_component = (
            (pynutil.delete("major_unit: \"") + pynini.closure(NEMO_NOT_QUOTE) + pynutil.delete("\""))
            | (pynutil.delete("minor_unit: \"") + pynini.closure(NEMO_NOT_QUOTE) + pynutil.delete("\""))
            | (pynutil.delete("minor_alt_unit: \"") + pynini.closure(NEMO_NOT_QUOTE) + pynutil.delete("\""))
        )
        ...
        graph_unit_money = pynini.closure(
            (number_component + delete_space + unit_only_component + delete_space)
        ) + pynini.closure(mandarin_currency_component)
        ...
        graph_range = (
            pynutil.delete("integer_pre: \"")
            + pynini.closure(NEMO_NOT_QUOTE)
            + pynutil.delete("\"")
            + delete_space
            + range_component
            + delete_space
            + pynutil.delete("integer_suf: \"")
            + pynini.closure(NEMO_NOT_QUOTE)
            + pynutil.delete("\"")
            + delete_space
            + (mandarin_currency_component | currency_component | unit_only_component)
        )
