...
class MoneyFst(GraphFst):
    def __init__(self, cardinal: GraphFst, decimal: GraphFst, deterministic: bool = True):
        ...
        currency_mandarin_component = (
            pynutil.insert("mandarin_currency: \"") + currency_mandarin + pynutil.insert("\"")
        )
        unit_components = (
            (pynutil.insert("major_unit: \"") + unit_major + pynutil.insert("\""))
            | (pynutil.insert("minor_unit: \"") + unit_minor + pynutil.insert("\""))
            | (pynutil.insert("minor_alt_unit: \"") + unit_minor_alt + pynutil.insert("\""))
        )
        ...
        morpheme = (
            pynini.accep("到")
            | pynini.accep("至")
            | pynini.closure(pynini.cross("-", "到"))
            | pynini.closure(pynini.cross("——", "到"))
            | pynini.closure(pynini.cross("~", "到"))
        )
        range_component = pynutil.insert("range: \"") + morpheme + pynutil.insert("\"")
        graph_range = (
            pynutil.insert("integer_pre: \"")
            + cardinal
            + pynutil.insert("\"")
            + pynutil.insert(" ")
            + range_component
            + pynutil.insert(" ")
            + pynutil.insert("integer_suf: \"")
            + cardinal
            + pynutil.insert("\"")
            + pynutil.insert(" ")
            + (currency_mandarin_component | unit_components)
        ) | (
            currency_component
            + pynutil.insert(" ")
            + pynutil.insert("integer_pre: \"")
