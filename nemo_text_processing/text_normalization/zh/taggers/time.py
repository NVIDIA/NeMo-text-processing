...
class TimeFst(GraphFst):
    ...
    def __init__(self, deterministic: bool=True):
        ...
        hour_minute_second = hour_component + symbol + pynutil.insert(' ') + minute_component + symbol + pynutil.insert(' ') + second_component
        ...
        hour_duration = pynini.accep('个点') | pynini.cross("個點", "個点") | pynini.accep('小时') | pynini.cross('小時', '小时') | pynini.accep('个小时') | pynini.cross('個小時','个小时') | pynini.accep('个钟头') | pynini.cross('個鐘頭', '个钟头')
        ...
        minute_component = pynutil.insert("minute: \"") + minute + (minute_clock | minute_duration) + pynutil.insert("\"")
        second_component = pynutil.insert("second: \"") + second + (second_clock | second_duration) + pynutil.insert("\"")
        ...
        graph_clock_period = hour_component | minute_component | second_component | hour_minute | hour_second | minute_second | clock_all
        
        # gramamr for time, back count; 五点差n分n秒
        backcount = pynutil.insert("verb: \"") + pynini.accep('差') + pynutil.insert("\"")
        graph_hour = (hour_component + pynutil.insert(' ') + backcount + pynutil.insert(' ') + minute_component) | (hour_component + pynutil.insert(' ') + backcount + pynutil.insert(' ') + second_component) | (hour_component + pynutil.insert(' ') + backcount + pynutil.insert(' ') + minute_component + pynutil.insert(' ') + second_component)
        ...
        am_component = (pynutil.insert("affix: \"") + prefix_am + pynutil.insert("\"")) | (pynutil.insert("affix: \"") + suffix_am + pynutil.insert("\""))  
        pm_component = (pynutil.insert("affix: \"") + prefix_pm + pynutil.insert("\"")) | (pynutil.insert("affix: \"") + suffix_pm + pynutil.insert("\"")) 
        
        graph_affix = ((am_component | pm_component) + pynutil.insert(' ') + (graph_colon | graph_clock_period | graph_backcount)) | ((graph_clock_period | graph_colon | graph_backcount) + pynutil.insert(' ') + (am_component | pm_component))
