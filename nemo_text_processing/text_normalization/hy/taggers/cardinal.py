...
class CardinalFst(GraphFst):
    ...
    def __init__(self):
        ...
        ties_unit = digits
        double_digits = ((pynini.string_map([("1", "տասը")]) | ties) +
                         pynutil.delete("0") | (pynini.string_map([("1", "տասն")]) | ties) + ties_unit)
        ...
        other_thousands = (
                (digits_no_one | double_digits | all_hundreds)
                + insert_space
                + pynutil.insert("հազար")
                + delete_separator
        )
        all_thousands = (
        ...
        all_millions = ((digits_to_hundreds + insert_space + pynutil.insert("միլիոն"))
                        + (millions_components @ (delete_zeros + pynini.closure(insert_space +
                                                                                digits_to_thousands, 0, 1))))
        ...
        all_billions = ((digits_to_hundreds + insert_space + pynutil.insert("միլիարդ"))
                        + (
                               ...
                               @ (
                                       delete_zeros
                                       + pynini.closure(insert_space + digits_to_millions, 0, 1)
                               )
                       ))

        final_graph = (zero | digits | double_digits | all_hundreds | all_thousands
                       | all_millions | all_billions)
