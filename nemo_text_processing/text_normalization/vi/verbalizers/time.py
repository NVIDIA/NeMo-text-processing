...
class TimeFst(GraphFst):
    ...
    def __init__(self, deterministic: bool = True):
        ...
        patterns = [
            hour_with_unit + pynini.closure(delete_space + zero_minute_component, 0, 1) + 
            pynini.closure(delete_space + zero_second_component, 0, 1) + optional_timezone + optional_preserve_order,
            
            hour_with_unit + delete_space + pynutil.insert(" ") + minute_with_unit + 
            pynini.closure(delete_space + zero_second_component, 0, 1) + optional_timezone + optional_preserve_order,
            
            hour_with_unit + delete_space + zero_minute_component + delete_space + 
            pynutil.insert(" ") + second_with_unit + optional_timezone + optional_preserve_order,
            
            hour_with_unit + delete_space + pynutil.insert(" ") + minute_with_unit + 
            delete_space + pynutil.insert(" ") + second_with_unit + optional_timezone + optional_preserve_order,
        ...
        if not deterministic:
            ...
            half_hour = pynutil.delete("minutes:") + delete_space + pynutil.delete("\"ba mươi\"") + pynutil.insert("rưỡi")
            half_hour_pattern = hour_with_unit + delete_space + pynutil.insert(" ") + half_hour + optional_timezone + optional_preserve_order
        ...
        self.fst = self.delete_tokens(self.graph + optional_preserve_order).optimize() @ remove_zero_minutes @ remove_zero_seconds 