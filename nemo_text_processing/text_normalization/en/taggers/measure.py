...
class MeasureFst(GraphFst):
    ...
    def __init__(
        ...
        math_graph = pynini.cross("-", "negative ").ques + cardinal.graph_with_and
        ...
        # alphanum_operator_alphanum_=_alphanum
        math = (
            (math_graph | NEMO_ALPHA)
            + delimiter
            + math_operations
            + delimiter
            + (math_graph | NEMO_ALPHA)
        ...
        # alphanum_=_alphanum_operator_alphanum
        math |= (
            (math_graph | NEMO_ALPHA)
            + delimiter
            + pynini.cross("=", "equals")
        ...
        # alphanum(_operator_alphanum)*
        math |= (math_graph | NEMO_ALPHA) + pynini.closure(
            delimiter + math_operations + delimiter + (math_graph | NEMO_ALPHA)
        )

        # alphanum(_operator_alphanum)*_=_alphanum
        math |= (
            (math_graph | NEMO_ALPHA)
            + pynini.closure(
                delimiter + math_operations + delimiter + (math_graph | NEMO_ALPHA)
        ...
        # alphanum_=_alphanum(_operator_alphanum)*
        math |= (
            (math_graph | NEMO_ALPHA)
            + delimiter
            + pynini.cross("=", "equals")
        ...
        # alphanum(_operator_alphanum)*_=_alphanum(_operator_alphanum)*
        math |= (
            (math_graph | NEMO_ALPHA)
            + pynini.closure(
                delimiter + math_operations + delimiter + (math_graph | NEMO_ALPHA)
