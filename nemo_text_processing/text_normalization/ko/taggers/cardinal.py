...
class CardinalFst(GraphFst):
    def __init__(self, deterministic: bool = True):
        ...
        graph_hundredthousand_component = (
            (NEMO_DIGIT ** 2 @ read_1_to_99) + pynutil.insert("만 ")
        ...
        graph_million_component = (
            (NEMO_DIGIT ** 3 @ read_100_to_999) + pynutil.insert("만 ")             
            + (NEMO_DIGIT ** 4 @ read_1_to_4_digits_improved)
            )
        ...
        graph_tenmillion_component = pynini.union(
            pynini.cross("0000", ""),
            first_part
        ) + (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits)
        ...
        read_8_digits = pynini.union(
            ...
            (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits) + pynutil.insert("만 ") +
            (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits)
        ).optimize()

        graph_hundredmillion_component = (
            (NEMO_DIGIT ** 1 @ read_1) + pynutil.insert("억 ") + read_8_digits
        )
        ...
        graph_billion_component = (
            (NEMO_DIGIT ** 2 @ read_1_to_99) + pynutil.insert("억 ") + read_8_digits_for_billion
        )
        ...
        def read_4_optional_with_unit(unit: str):
            return pynini.union(
                pynini.cross("0000", ""),  
                (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits) + pynutil.insert(unit)
            )
        
        graph_trillion_component = (
            (NEMO_DIGIT ** 1 @ read_1) + pynutil.insert("조 ") +
        ...
        graph_ten_trillion_component = (
            (NEMO_DIGIT ** 2 @ read_1_to_99) + pynutil.insert("조 ") +
            read_4_optional_with_unit("억 ") +
            read_4_optional_with_unit("만 ") +
            (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits)
        )
        ...
        graph_hundred_trillion_component = (
            (NEMO_DIGIT ** 3 @ read_100_to_999) + pynutil.insert("조 ") +
            read_4_optional_with_unit("억 ") +
            read_4_optional_with_unit("만 ") +
            (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits)
        )
        ...
        graph_thousand_trillion_component = (
            (NEMO_DIGIT ** 4 @ read_1000_to_9999) + pynutil.insert("조 ") +
            read_4_optional_with_unit("억 ") +
            read_4_optional_with_unit("만 ") +
            (NEMO_DIGIT ** 4 @ read_optional_1_to_4_digits)
        )
        ...
        graph_quadrillion_component = (
            (NEMO_DIGIT ** 1 @ read_1) + pynutil.insert("경 ") +
