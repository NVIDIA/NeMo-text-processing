...
def evaluate(preds: List[str], labels: List[str], input: Optional[List[str]] = None, verbose: bool = True) -> float:
    ...
    for i in range(nums):
        ...
        if pred_norm == label_norm:
            ...
            if input:
                print(f"input: {json.dumps(input[i], ensure_ascii=False)}")
            print(f"gold: {json.dumps(label_norm, ensure_ascii=False)}")
            print(f"pred: {json.dumps(pred_norm, ensure_ascii=False)}")
    return acc / nums
