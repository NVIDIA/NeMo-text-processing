...
class RomanFst(GraphFst):
    """
    Finite state transducer for classifying roman numbers in Greek,
        e.g. "IV" -> tokens { roman { integer: "τέσσερα" } }
        e.g. "Elizabeth II" -> tokens { roman { key_the_ordinal: "Elizabeth" integer: "δεύτερη" } }
        e.g. "Κεφάλαιο V" -> tokens { roman { key_cardinal: "Κεφάλαιο" integer: "πέντε" } }
    ...
    Args:
        ...
        lm: if True will provide a single transduction option for language model
    """

    def __init__(self, cardinal=None, ordinal=None, deterministic: bool = True, lm: bool = False):
        ...
        # graph for teen Roman numerals (I-XIX)
        graph_teens = pynini.string_map([x[0] for x in roman_dict[1:ordinal_limit]]).optimize()

        # Names (male + female)
        names = get_names()

        # Name + teen Roman → ordinal ("Elizabeth II" → "Ελισάβετ δεύτερη")
        graph = (
        ...
        # Key words + Roman → cardinal ("Κεφάλαιο V" → "Κεφάλαιο πέντε")
        key_words = []
