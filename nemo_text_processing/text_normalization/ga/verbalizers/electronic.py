...
class ElectronicFst(GraphFst):
    ...
    Finite state transducer for verbalizing electronic
        e.g. electronic { username: "abc" domain: "hotmail.com" } -> "a b c ag hotmail ponc com"
                                                           -> "a b c ag h o t m a i l ponc c o m"
                                                           -> "a b c ag hotmail ponc c o m"
                                                           -> "a b c ag h o t m a i l ponc com"
    Args:
    ...
    def __init__(self, deterministic: bool = True):
        ...
        graph_digit = pynutil.insert("a ") + (digit_no_zero | zero)
        ...
        self.graph = (pynini.closure(protocol + pynini.accep(" "), 0, 1) + domain) | (
            user_name + pynini.accep(" ") + pynutil.insert("ag ") + domain
        )
