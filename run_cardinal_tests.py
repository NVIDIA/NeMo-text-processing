...
def main():
    if len(sys.argv) < 4:
        print('Usage: python run_cardinal_tests.py <lang> <tn|itn> <path-to-test_cases_cardinal.txt>')
        sys.exit(2)
...
def main(): 
    lang, direction, path = sys.argv[1], sys.argv[2], sys.argv[3] 
    base = "text_normalization" if direction == "tn" else "inverse_text_normalization" 
    tagger = importlib.import_module( 
        f"nemo_text_processing.{base}.{lang}.taggers.cardinal").CardinalFst().fst 
    verbalizer = importlib.import_module( 
        f"nemo_text_processing.{base}.{lang}.verbalizers.cardinal").CardinalFst().fst 
