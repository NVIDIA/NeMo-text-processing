...
//         sh 'git config --global user.email "$(whoami)@$(hostname)"'
    ...
    stage('L0: TN/ITN Tests CPU') {
      ...
      environment{
       ...
       TN_CACHE = """${sh(
                returnStdout: true,
                script: 'bash /home/jenkinsci/scripts/get_cache_folder.sh /home/jenkinsci/TestData/text_norm/ci/grammars/ en'
            )}"""
      ...
      parallel {
        stage('En TN grammars') {
          steps {
            sh 'CUDA_VISIBLE_DEVICES="" python nemo_text_processing/text_normalization/normalize.py --text="1" --cache_dir $TN_CACHE'
          }
        }
        stage('En ITN grammars') {
          steps {
            sh 'CUDA_VISIBLE_DEVICES="" python nemo_text_processing/inverse_text_normalization/inverse_normalize.py --language en --text="twenty" --cache_dir $TN_CACHE'
          }
        }
        stage('Test En non-deterministic TN & Run all En TN/ITN tests (restore grammars from cache)') {
          steps {
            sh 'CUDA_VISIBLE_DEVICES="" pytest tests/nemo_text_processing/en/ -m "not pleasefixme" --cpu --tn_cache_dir $TN_CACHE'
          }
        }
      ...
      parallel {
        stage('L2: Eng TN') {
          steps {
            ...
            cd tools/text_processing_deployment && python pynini_export.py --output=$NORM_OUTPUT_DIR --grammars=tn_grammars --cache_dir $TN_CACHE --language=en && ls -R $NORM_OUTPUT_DIR && echo ".far files created "|| exit 1'
        ...
        stage('L2: Eng ITN export') {
          steps {
            ...
            cd tools/text_processing_deployment && python pynini_export.py --output=$DENORM_OUTPUT_DIR --grammars=itn_grammars --cache_dir $TN_CACHE --language=en && ls -R $DENORM_OUTPUT_DIR && echo ".far files created "|| exit 1'
