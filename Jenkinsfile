pipeline {
  ...
  environment {
    ...
    IT_TN_CACHE='/home/jenkinsci/TestData/text_norm/ci/grammars/06-29-23-0'
    ...
    SV_TN_CACHE='/home/jenkinsci/TestData/text_norm/ci/grammars/06-29-23-0'
  ...
  stages {
    ...
    stage('L0: Create IT TN/ITN Grammars') {
      when {
        anyOf {
          branch 'main'
          changeRequest target: 'main'
      ...
      parallel {
        stage('L0: IT TN grammars') {
         steps {
            sh 'CUDA_VISIBLE_DEVICES="" python nemo_text_processing/text_normalization/normalize.py --lang=it --text="100" --cache_dir ${IT_TN_CACHE}'
          }
        }
       // stage('L0: IT ITN grammars') {
       //   steps {
       //     sh 'CUDA_VISIBLE_DEVICES="" python nemo_text_processing/inverse_text_normalization/inverse_normalize.py --lang=it --text="cento" --cache_dir ${IT_TN_CACHE}'
       //   }
       // }
      }
    }
...
// L1 Tests starts here
    stage('L1: TN/ITN Tests CPU') {
      ...
      stages {
        ...
        stage('L1: Run all FR TN/ITN tests (restore grammars from cache)') {
          steps {
            sh 'CUDA_VISIBLE_DEVICES="" pytest tests/nemo_text_processing/it/ -m "not pleasefixme" --cpu --tn_cache_dir ${IT_TN_CACHE}'
          }
        }
