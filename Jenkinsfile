pipeline {
  ...
  environment {
    ...
    EN_TN_CACHE='/home/jenkinsci/TestData/text_norm/ci/grammars/07-19-24-0'
  ...
  stages {
    ...
    stage('L0: Create EN TN/ITN Grammars') {
      ...
      parallel {
        stage('L0: Test utils') {
          steps {
            sh 'pip uninstall pytest && pip install pytest && CUDA_VISIBLE_DEVICES="" pytest tests/nemo_text_processing/audio_based_utils/ --cpu'
          }
        }
