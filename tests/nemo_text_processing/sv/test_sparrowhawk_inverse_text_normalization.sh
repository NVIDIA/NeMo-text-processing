...
runtest () {
  ...
  while read testcase; do
    ...
    denorm_pred="$(echo -e "${denorm_pred}" | sed -e 's/^[[:space:]]*//' -e 's/[[:space:]]*$//' -e 's/\xC2\xA0/ /g')"
...
testITNCardinal() {
  input=$PROJECT_DIR/sv/data_inverse_text_normalization/test_cases_cardinal.txt
  runtest $input
}

testITNDate() {
  input=$PROJECT_DIR/sv/data_inverse_text_normalization/test_cases_date.txt
  runtest $input
}

testITNDecimal() {
  input=$PROJECT_DIR/sv/data_inverse_text_normalization/test_cases_decimal.txt
  runtest $input
}

testITNOrdinal() {
  input=$PROJECT_DIR/sv/data_inverse_text_normalization/test_cases_ordinal.txt
  runtest $input
}
