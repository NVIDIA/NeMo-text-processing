...
class TestCardinal:
    ...
    def test_denorm_project(self, test_input, expected):
        pred = self.inverse_normalizer_sv_projecting.inverse_normalize(test_input, verbose=True)
        if test_input == expected:
            assert pred == expected
        else:
            assert pred == f'{expected}[{test_input}]'

    normalizer_sv_projecting = Normalizer(input_case='cased', lang='sv', project_input=True, cache_dir=CACHE_DIR, overwrite_cache=False)
    ...
    def test_norm_project(self, test_input, expected):
        pred = self.normalizer_sv_projecting.normalize(test_input, verbose=True)
        if test_input == expected:
            assert pred == expected
        else:
            assert pred == f'{expected}[{test_input}]'
