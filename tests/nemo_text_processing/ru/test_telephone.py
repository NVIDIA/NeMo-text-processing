...
class TestTelephone:
    ...
    @pytest.mark.run_only_on('CPU')
    @pytest.mark.unit
    def test_norm_telephone_project_input(self, expected, test_input):
        pred = self.normalizer_project.normalize(test_input)
        assert_projecting_output(pred, expected, test_input)

    @parameterized.expand(parse_test_case_file('ru/data_inverse_text_normalization/test_cases_telephone.txt'))
    @pytest.mark.run_only_on('CPU')
    @pytest.mark.unit
