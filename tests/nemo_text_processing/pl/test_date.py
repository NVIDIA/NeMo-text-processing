...
class TestDate:
    ...
    @parameterized.expand(parse_test_case_file("pl/data_text_normalization/test_cases_date.txt"))
    ...
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_case_coordinated_graphs(self):
        cardinal = CardinalFst(deterministic=True)
        ordinal = OrdinalFst(deterministic=True)
    ...
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_bare_year_is_only_a_nondeterministic_date(self):
        cardinal = CardinalFst(deterministic=False)
        ordinal = OrdinalFst(deterministic=False)
        date = DateFst(cardinal, ordinal, deterministic=False)
        assert rewrite.one_top_rewrite("1832", date.year_graphs["mi_sg_gen"]) == (
            'year: "tysiąc osiemset trzydziestego drugiego"'
        )
