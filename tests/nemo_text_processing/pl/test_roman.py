...
class TestRoman:
    ...
    @parameterized.expand(parse_test_case_file("pl/data_text_normalization/test_cases_roman.txt"))
    ...
    @pytest.mark.run_only_on("CPU")
    @pytest.mark.unit
    def test_two_component_name_inflection(self):
        roman = RomanFst(OrdinalFst())
        assert rewrite.one_top_rewrite("Franciszka Józefa I", roman.graphs["mp_sg_gen"]) == (
            "Franciszka Józefa pierwszego"
        )
