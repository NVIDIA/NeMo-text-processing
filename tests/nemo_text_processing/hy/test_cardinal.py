...
class TestCardinal:
    ...
    @parameterized.expand(parse_test_case_file('hy/data_inverse_text_normalization/test_cases_cardinal.txt'))
    ...
    @parameterized.expand(parse_test_case_file('hy/data_text_normalization/test_cases_cardinal.txt'))
    @pytest.mark.run_only_on('CPU')
    @pytest.mark.unit
    ...
    def test_denorm_project(self, test_input, expected):
        pred = self.inverse_normalizer_hy_projecting.inverse_normalize(test_input, verbose=True)
        assert_projecting_output(pred, expected, test_input)

    normalizer_hy_projecting = Normalizer(
        lang='hy', project_input=True, cache_dir=CACHE_DIR, overwrite_cache=False, input_case='lower_cased'
    )
