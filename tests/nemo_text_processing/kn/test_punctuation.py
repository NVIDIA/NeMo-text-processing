...
class TestPunctuation:
    ...
    @parameterized.expand(parse_test_case_file('kn/data_text_normalization/test_cases_punctuation_match_input.txt'))
    @pytest.mark.run_only_on('CPU')
    @pytest.mark.unit
    def test_norm_python_punct_post_process(self, test_input, expected):
        pred = self.normalizer.normalize(test_input, verbose=True, punct_post_process=True)
