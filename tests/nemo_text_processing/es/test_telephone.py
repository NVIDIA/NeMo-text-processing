...
class TestTelephone:
    ...
    @parameterized.expand(parse_test_case_file('es/data_inverse_text_normalization/test_cases_telephone_cased.txt'))
    @pytest.mark.run_only_on('CPU')
    @pytest.mark.unit
    ...
    def test_norm_project(self, test_input, expected):
        ...
        if test_input in expected:
            ...
            assert any(pred == f'{exp}[{test_input}]' for exp in expected)
