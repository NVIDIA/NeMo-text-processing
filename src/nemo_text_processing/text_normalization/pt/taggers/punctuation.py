...
from nemo_text_processing.text_normalization.pt.graph_utils import NEMO_NOT_SPACE, NEMO_SIGMA, GraphFst
from nemo_text_processing.text_normalization.pt.utils import get_abs_path, load_labels


class PunctuationFst(GraphFst):
    """
    Finite state transducer for classifying punctuation (pt-BR pipeline).
    Mirrors the English punctuation tagger; whitelist symbols are excluded via data/whitelist/symbol.tsv.
    """
    ...
    def __init__(self, deterministic: bool = True):
        ...
        whitelist_rows = load_labels(symbol_path) if os.path.isfile(symbol_path) else []
        whitelist_symbols = [x[0] for x in whitelist_rows if x]
